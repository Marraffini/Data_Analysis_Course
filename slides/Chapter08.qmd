...
Whenever you perform a hypothesis test, there is always a chance of committing a type I error aka 'false positive'.


When we perform one hypothesis test, the type I error rate is equal to the significance level ($\alpha$), however, when we conduct multiple hypothesis tests at once, the probability of getting a false positive increases.
...
Family-wise error rate = $1 – (1-\alpha)^n$
...
There are several procedures available that allow use to make multiple comparisons of a set of treatment means while still, under some assumptions, assuring that the overall confidence level associated with all the comparisons remain at or above the specified $100(1-\alpha) \%$ level.
...
-   Tukey HSD (Honest Significant Differences) allows pairwise
    comparison of treatment means 
-   Tukey's test is essentially a t-test, except that it corrects for family-wise error rate.
-   HSD is the least amount that means must vary from each other to be significantly different
-   often called a post-hoc test



$HSD = q * \sqrt{\frac{MSE}{n}}$

q is a constant from studentized range q table

MSE is the mean square error within

n = the number in each category 

## Tukey HSD: Fabric Example
...
Refers to the process of adjusting the alpha ($\alpha$) level for a family of statistical tests so that we control for the probability of committing a type I error.

The formula for a Bonferroni Correction is as follows:

$\alpha_{new} = \alpha_{original} / n$

where:

$\alpha_{original}$: The original $\alpha$ level set *a priori*
...
-   check that all anova assumptions are satisfied
-   specify $\alpha$, the overall significance level
-   specify the significance level for each comparison as $\alpha / n $
-   summaries the results as confidence intervals for each pair of means and associated significance in tables or plots

## Bonferroni Correction: Fabric Example

Do the different fabric types have different burn times?

```{r, echo=TRUE}
# make an object to hold all the pvalues from each ttest
pvals <- c(
  t.test(burntime ~ fabric, 
         data = fabric |> filter(fabric == "Fabric 1" | fabric == "Fabric 2" ))$p.value,
  t.test(burntime ~ fabric, data = fabric |> filter(fabric == "Fabric 1" | fabric == "Fabric 3" ))$p.value,
  t.test(burntime ~ fabric, data = fabric |> filter(fabric == "Fabric 1" | fabric == "Fabric 4" ))$p.value,
    t.test(burntime ~ fabric,  data = fabric |> filter(fabric == "Fabric 2" | fabric == "Fabric 3" ))$p.value,
    t.test(burntime ~ fabric, data = fabric |> filter(fabric == "Fabric 2" | fabric == "Fabric 4" ))$p.value,
  t.test(burntime ~ fabric,  data = fabric |> filter(fabric == "Fabric 3" | fabric == "Fabric 4" ))$p.value
)

# adjust the pvalues using bonferroni 
p.adjust(pvals, method = "bonferroni")

```

The first three comparisons are significant. Fabric one is different from all the other fabrics in terms of burn time.

This is a similar results to the Tukey's procedure but this my not always be the case.

## Bonferroni or Tukeys

Should be used when making multiple comparisons
...
Note: If the sample sizes are not equal, you can use a modified version of the test known as the Tukey-Kramer test. We won't cover this modification.

The Bonferroni post-hoc test should be used when you have a set of planned comparisons you would like to make beforehand. 

All of this should be decided *before* conducting an experiment/analysis.
...
## Interaction Plot for ToothGrowth data
...
Mann-Whitney test: allows to compare 2 groups under the non-normality assumption. Often referred to as the Wilcoxon Rank-Sum Test.
...
```{r}
#| echo: true
# Mann Whitney is often referred to as the Wilcoxon Rank-Sum Test, hence the function.
