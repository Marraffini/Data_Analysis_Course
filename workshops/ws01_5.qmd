...
conserves only the needed columns. Try
...
Use the above three tables you generated to describe the Rangitikei dataset. What other information or tables might be useful?
...
```{r}
data.na <-data.frame(a=c(1,NA,5),b=c(3,2,2))

colMeans(data.na,na.rm=TRUE)

colMeans(na.omit(data.na))

```
Why is there a difference between colMeans(data.na, na.rm=TRUE) and colMeans(na.omit(data.na)) for column b?
